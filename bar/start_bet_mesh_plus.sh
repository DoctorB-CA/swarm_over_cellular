#!/bin/bash
set -euo pipefail

# =========================
# wlan0 -> IBSS + batman-adv mesh (10.0.0.X/24 on bat0)
# wlan1 -> stays usable/managed (we only “detach” wlan0)
# =========================

IFACE_MESH="${IFACE_MESH:-wlan0}"
IFACE_KEEP="${IFACE_KEEP:-wlan1}"

SSID="${SSID:-call-code-mesh}"
FREQ_MHZ="${FREQ_MHZ:-2412}"     # ch1=2412, ch6=2437, ch11=2462
MTU="${MTU:-1468}"
GW_MODE="${GW_MODE:-client}"

# Pick node id (last octet) from arg1 or NODE_ID env
NODE_ID="${1:-${NODE_ID:-}}"
if [[ -z "${NODE_ID}" ]]; then
  echo "Usage: $0 <NODE_ID 1-254>"
  echo "Example: $0 2   -> bat0 gets 10.0.0.2/24"
  exit 1
fi
if ! [[ "$NODE_ID" =~ ^[0-9]+$ ]] || (( NODE_ID < 1 || NODE_ID > 254 )); then
  echo "NODE_ID must be an integer 1..254"
  exit 1
fi
MESH_IP="10.0.0.${NODE_ID}/24"

log(){ echo "[*] $*"; }

# ---- sanity
ip link show "$IFACE_MESH" >/dev/null 2>&1 || { echo "No such iface: $IFACE_MESH"; exit 1; }
command -v iw >/dev/null || { echo "Missing: iw"; exit 1; }
command -v batctl >/dev/null || { echo "Missing: batctl (sudo apt install batctl)"; exit 1; }

log "Detaching $IFACE_MESH from DHCP managers (keeping $IFACE_KEEP untouched)..."
if command -v dhcpcd >/dev/null 2>&1; then
  sudo dhcpcd -k "$IFACE_MESH" 2>/dev/null || true   # release lease if any
  sudo dhcpcd -x "$IFACE_MESH" 2>/dev/null || true   # stop managing ONLY this iface
fi

log "If NetworkManager exists: mark $IFACE_MESH unmanaged (does not affect $IFACE_KEEP)..."
if command -v nmcli >/dev/null 2>&1; then
  sudo nmcli dev set "$IFACE_MESH" managed no 2>/dev/null || true
fi

log "Stopping any per-interface wpa_supplicant instance for $IFACE_MESH (leave others running)..."
sudo systemctl stop "wpa_supplicant@${IFACE_MESH}.service" 2>/dev/null || true
# Kill only processes that explicitly mention -i<wlan0> (won't touch wlan1 process)
sudo pkill -f "wpa_supplicant.*-i${IFACE_MESH}\b" 2>/dev/null || true

log "Deleting Wi-Fi Direct (P2P) virtual ifaces tied to $IFACE_MESH (common source of -16 busy)..."
for p2p in "p2p-dev-${IFACE_MESH}" "p2p-${IFACE_MESH}-0" "p2p-${IFACE_MESH}-1"; do
  sudo iw dev "$p2p" del 2>/dev/null || true
done

log "Unblocking Wi-Fi (rfkill) ..."
sudo rfkill unblock wifi 2>/dev/null || true

log "Switching $IFACE_MESH to IBSS and joining '$SSID' @ ${FREQ_MHZ}MHz..."
sudo ip link set dev "$IFACE_MESH" down
sudo iw dev "$IFACE_MESH" set type ibss
sudo ip link set dev "$IFACE_MESH" up
sudo iw dev "$IFACE_MESH" ibss join "$SSID" "$FREQ_MHZ" fixed-freq

log "Loading batman-adv and attaching $IFACE_MESH -> bat0..."
sudo modprobe batman-adv
sudo batctl if add "$IFACE_MESH"
sudo ip link set dev bat0 up
sudo ip link set mtu "$MTU" dev bat0
sudo batctl gw_mode "$GW_MODE"

log "Assigning IP $MESH_IP to bat0..."
sudo ip addr flush dev bat0
sudo ip addr add "$MESH_IP" dev bat0

log "Done."
log "Verify:"
echo "    iw dev $IFACE_MESH info | grep -i type"
echo "    ip -br addr show bat0"
echo "    sudo batctl o"

